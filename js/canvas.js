...
function Circle(x, y, radius, color, text) {
    ...
    this.rotationRadius = 1.0 + Math.random() * 2.0;
...
Circle.prototype.draw = function() {
    var realRadius = (canvas.width > 600 ? (this.radius * 1.4) : (this.radius * (canvas.width / 600)));

    var realX = ((this.x) * canvas.width + parallaxOffset.x);
    var realY = ((this.y) * canvas.height + parallaxOffset.y);
    ...
    ctx.fillStyle = "white";
    var font = (realRadius/3).toString() +"px 'Noto Sans'";
    ctx.font = font;

    // draw text
    var width  = ctx.measureText(this.text).width;
    var height = ctx.measureText("w").width;
    ctx.fillText(this.text, realX - (width/2), realY + (height/2));
...
$(function() {
    ...
    for (var i = 0; i < circleInfo.length; i++) {
        circles.push(new Circle(circleInfo[i].x, circleInfo[i].y, circleInfo[i].radius, circleInfo[i].color, circleInfo[i].text));
    }
